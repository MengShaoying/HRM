...
<head>
    <meta charset='UTF-8'>
    <title>{{ project::$entity_display_name }}[{{ $project->id }}]修改</title>
    <style>
     table {
    ...
    <tr>
        ...
        <td>
            ...
            <select name='{{ $struct }}'>
                ...
                <option value='{{ $key }}' {{ $key === $project->{$struct}?'selected':'' }}>{{ $value }}</option>
                @endforeach
            </select>
            @else
            <input type='{{ $type }}' name='{{ $struct }}' value='{{ $project->{$struct} }}'>
            @endif
        </td>
        @endforeach
        <td>
